...
OUTPUT_DIR = "outputs/shahar_zach_comparison/"
...
def run_fal_instantid(photo_path, i):
    try:
        ...
        if img_url:
            ...
            filename = f"Shahar_Zach_InstantID_V{i}.png"
    ...
    try:
        ...
        payload = {
            ...
            "stages": [
                ...
                {"type": "DIFFUSION", "diffusion": {
                    ...
                    "sdModel": SD_MODEL_ID, "steps": 35, "cfgScale": 8, "denoisingStrength": 0.45
                }}
            ]
        }
        ...
        for _ in range(60):
            ...
            if status_res["job"]["status"] == "SUCCESS":
                ...
                content = requests.get(img_url).content
                filename = f"Shahar_Zach_Tensor_V{i}.jpg"
                with open(os.path.join(OUTPUT_DIR, filename), "wb") as f:
                    f.write(content)
                return True
            time.sleep(5)
...
def main():
    ...
    print("Generating InstantID variants...")
    run_fal_instantid(PHOTO_PATH, 1)
    run_fal_instantid(PHOTO_PATH, 2)
    
    print("Generating Tensor variants...")
