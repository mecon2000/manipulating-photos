...
> **STATUS: DONE (2026-08-08).** `studio/recipes.py` (versioned JSON in `shared/studio/recipes/` + thumbnail; general/specific marks; rename/trash/update_step_params/from_favorite), `studio/apply_recipe.py` (batch CLI registered as manifest action `studio-apply-recipe` — **runs through the hub job runner**, verified end-to-end: hub POST → job done; per-photo Studio sessions pre-created with `recipe` recorded, so contact-sheet taps open delta-ready sessions), app pages `/recipes` + `/batches` with full CRUD API, `POST /api/recipes/{slug}/apply` picks N candidates (optional model-name filter) and kicks the hub job, favorites→recipe importer (`/api/recipes/import-favorite` + `/api/favorites` picker). Agent: `save_recipe`, `update_recipe` (fold deltas); recipe-seeded sessions get a system-prompt block telling the buddy to watch for recurring deltas across the batch and propose folding. Frontend: recipes grid (rename/delete/apply-to dialog), batches contact sheet, save-recipe dialog with general/specific checkboxes. Note: non-studio favorites import as single-step recipes with underscore param names that may not match registry dashes — apply errors visibly if so; studio-type favorites import their full chain cleanly.
...
>
> **Post-plan (2026-08-08, first live feedback):** builtin **inpaint** step (Flux via fal, not registry-bound — "add flowers at #1"-type generative edits) with black-output NSFW detection and **auto style-matching** (a local-VLM style phrase is appended to bare prompts so edits inherit realistic/anime/painterly look); agent tools `sam_mask` (existing objects) + `region_mask` (blob for adding into empty areas — SAM on empty background segments the whole BG, learned live); Pan-mode gesture fix (two-finger pinch+pan and one-finger drag now coexist — Konva drag is stopped when a pinch starts, midpoint drift translates).
>
