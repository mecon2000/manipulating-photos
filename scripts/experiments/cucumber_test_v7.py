...
def get_resource_id(image_url):
    ...
    requests.put(put_url, data=img_data, headers=upload_headers)
    return resource_id
...
if resource_id:
    ...
    # Strength 0.0001 as Ronnie suggested to see if it works
    job_id = create_job(resource_id, 0.0001)
    if job_id:
        print(f"Job ID: {job_id}")
        url = wait_for_job(job_id)
        print(f"CUCUMBER_RESULT_V7: {url}")
