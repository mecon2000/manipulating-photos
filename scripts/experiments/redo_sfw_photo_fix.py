...
PHOTO_PATH = "user_provided_sfw.jpg"
...
STRENGTH = 0.55
OUTPUT_DIR = "outputs/sfw_photo_fix/"
...
def run_tensor_prompt():
    try:
        ...
        while True:
            ...
            elif res["job"]["status"] == "FAILED": 
                print(f"Tensor failed: {res}")
                return None
            time.sleep(5)
...
def run_fal_flux_img2img():
    try:
        print("Starting Fal Flux Img2Img...")
...
def run_fal_face_swap(base_image_url):
    try:
        print(f"Starting Fal Face Swap on {base_image_url[:50]}...")
...
def main():
    ...
    for name, func in methods.items():
        ...
        if url:
            ...
            ext = "png" if "InstantID" in name else "jpg"
            filename = f"SFW_photo_{name}.{ext}"
            ...
            if "Fal_Flux" in name:
                ...
                if swap_url:
                    ...
                    swap_filename = f"SFW_photo_Method_4_Fal_FaceSwap_on_Flux.jpg"
