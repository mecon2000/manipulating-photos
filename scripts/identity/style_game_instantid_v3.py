...
OUTPUT_DIR = "outputs/style_game_batch_v3/"
...
def run_fal_instantid(photo_path, strength, style_variant):
    try:
        ...
        data = {
            ...
            "prompt": f"Professional photography, {style_variant}. {IDENTITY_PROMPT}",
            "strength": strength,
...
def main():
    ...
    with open(photo, "rb") as f_src:
        with open(os.path.join(OUTPUT_DIR, "Omry_Ksenia_BLD_9490E_Original.jpg"), "wb") as f_dst:
            f_dst.write(f_src.read())

    for i, style in enumerate(styles, 1):
        print(f"Running variation {i}...")
        url = run_fal_instantid(photo, strength, style)
        if url:
            img_data = requests.get(url).content
            filename = f"Omry_Ksenia_BLD_9490E_Nebula_V{i}.png"
