...
def draw_element(canvas, element, color, pos, size, w, h):
    if element == "none":
        return canvas
...
def main():
    ...
    p.add_argument("--local-output-dir", default="~/.openclaw/workspace/shared")
    args = p.parse_args()
    ...
    mask_pil, mask_info = build_mask(source, affect="subject", output_dir=output_dir, feather=0.3)
    ...
    finals_dir = os.path.join(os.path.expanduser(args.local_output_dir), "finals")
